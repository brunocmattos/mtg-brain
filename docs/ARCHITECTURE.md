...
**Histórico / decisão:** uma versão anterior embutia um chat no app com LLM local (Ollama, via uma ferramenta `run_sql`). Foi **removida** — o modelo local (14b) não tinha qualidade suficiente e consumia disco/RAM. Tradução de pergunta → SQL para dados estruturados ("comandantes UB de dreno até US$5", "combos com Gravecrawler") é mais **precisa e auditável** que busca por similaridade; e fazê-la via Claude Code dá qualidade real sem custo. **Busca semântica (pgvector) — implementada:** cada carta tem um embedding 768-d (texto = nome + tipo + oráculo) gerado por **fastembed** (`BAAI/bge-base-en-v1.5`, ONNX, local e grátis — bem mais leve que um LLM) na coluna `cards.embedding`, com índice **HNSW** (cosine). A query usa o prefixo de instrução do bge (uso assimétrico query→passage); o corpus fica sem prefixo. `GET /api/cards/semantic?q=` embeda a query e ordena por proximidade de vetor — busca por *significado*, complementar ao SQL exato (melhor para preço/cor/legalidade/combos). O modelo roda no container do app (Python 3.12); `ingest embeddings` recria a coluna na dimensão do modelo e (re)popula. Filtra a `commander='legal'` (corta cartas de piada/ilegais). **Qualidade (honesta):** boa em intenções diretas (sacrifício, reanimação, proteção); intenções **compostas** ("punir o oponente POR comprar") ainda erram — limite de *embedding puro*, não de tamanho do modelo (bge-base ~= bge-small nesse caso). O lever real seria **busca híbrida** (semântica + palavra-chave) ou um **reranker** — fica no roadmap.
