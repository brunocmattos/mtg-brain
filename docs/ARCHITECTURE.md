...
**Histórico / decisão:** uma versão anterior embutia um chat no app com LLM local (Ollama, via uma ferramenta `run_sql`). Foi **removida** — o modelo local (14b) não tinha qualidade suficiente e consumia disco/RAM. Tradução de pergunta → SQL para dados estruturados ("comandantes UB de dreno até US$5", "combos com Gravecrawler") é mais **precisa e auditável** que busca por similaridade; e fazê-la via Claude Code dá qualidade real sem custo. **Busca semântica (pgvector) — implementada:** cada carta tem um embedding 384-d (texto = nome + tipo + oráculo) gerado por **fastembed** (`BAAI/bge-small-en-v1.5`, ONNX, local e grátis — bem mais leve que um LLM) na coluna `cards.embedding`, com índice **HNSW** (cosine). `GET /api/cards/semantic?q=` embeda a query e ordena por proximidade de vetor — busca por *significado*, complementar ao SQL exato (que segue melhor para preço/cor/legalidade/combos). O modelo roda no container do app (Python 3.12); `ingest embeddings` (re)popula os vetores. Filtra a `commander='legal'` (corta cartas de piada/ilegais). Qualidade: boa em intenções diretas (sacrifício, reanimação), mais ruidosa em intenções compostas — limite de um modelo pequeno; dá pra subir pra `bge-base` se quiser mais precisão.
